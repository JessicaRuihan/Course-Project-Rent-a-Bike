...
def clean_data(data: List[list]) -> None:
    ...
    for sublist in data:
        for char in sublist:
            if is_number(char) == flag:
                if '.' not in char or '.0' in char:
                    sublist[sublist.index(char)] = int(float(char))
                else:
                    sublist[sublist.index(char)] = float(char)
...
def has_kiosk(station: "Station") -> bool:
    ...
    False
    """
    return bool(NO_KIOSK not in station[NAME])
...
def get_station_info(station_id: int, stations: List["Station"]) -> list:
    ...
    result = []
    for lst in stations:
        if station_id == lst[ID]:
            result.append(lst[NAME])
            result.append(lst[BIKES_AVAILABLE])
            result.append(lst[DOCKS_AVAILABLE])
            result.append(bool(NO_KIOSK not in lst[NAME]))
    return result
...
def get_nearest_station(lat: float, lon: float, with_kiosk: bool,
    ...
    if with_kiosk == false:
        for lst in stations:
            diff.append(get_lat_lon_distance(lat, lon, lst[LATITUDE], lst[LONGITUDE]))
        return stations[diff.index(min(diff))][0]
    elif with_kiosk == true:
        for lsts in stations:
            if bool(NO_KIOSK in lsts[NAME]):
                stations.remove(lsts)
                for lstss in stations:
                    diff.append(get_lat_lon_distance(lat, lon, lstss[LATITUDE], 
                                                     lstss[LONGITUDE]))                                        
                return stations[diff.index(min(diff))][0]
    return None   
...
def rent_bike(station_id: int, stations: List["Station"]) -> bool:
    ...
    for sublist in stations:
        if station_id in sublist:
            if sublist[BIKES_AVAILABLE] >= 1:
                sublist[BIKES_AVAILABLE] = sublist[BIKES_AVAILABLE] - 1 
                sublist[DOCKS_AVAILABLE] = sublist[DOCKS_AVAILABLE] + 1
                return True
            else:
                return False
    return None
...
def return_bike(station_id: int, stations: List["Station"]) -> bool:
    ...
    """
    for lst in stations:
        if station_id in lst:
            if lst[DOCKS_AVAILABLE] >= 1:
                lst[BIKES_AVAILABLE] = lst[BIKES_AVAILABLE] + 1 
                lst[DOCKS_AVAILABLE] = lst[DOCKS_AVAILABLE] - 1  
                return True
            else:
                return False
    return None
...
def redistribute_bikes(stations: List["Station"]) -> int:
    ...
    lst2 = []
    for lst in stations:  
        lst1.append(lst[BIKES_AVAILABLE])
        lst2.append(lst[CAPACITY])
    total_bikes = sum(lst1)
    total_capacity = sum(lst2)
    target_percent = total_bikes / total_capacity
    for lsts in stations:
        orginal_bike_docks = lsts[BIKES_AVAILABLE] + lsts[DOCKS_AVAILABLE]
        lsts.pop(BIKES_AVAILABLE)
        lsts.insert(BIKES_AVAILABLE, round(target_percent * lsts[CAPACITY])) 
        lsts.pop(DOCKS_AVAILABLE)
        lsts.insert(DOCKS_AVAILABLE, orginal_bike_docks - lsts[BIKES_AVAILABLE])
    lst3 = []
    lst4 = [] 
    lst5 = []
    for lstss in stations:
        lst3.append(lstss[BIKES_AVAILABLE])
        lst4.append(lstss[DOCKS_AVAILABLE])
        lst5.append(lstss[CAPACITY])
    sum_bikes = sum(lst3 + lst4)
    sum_cap = sum(lst5)
    return sum_bikes - sum_cap
